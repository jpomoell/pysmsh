...
"""Staggered finite difference operations
"""
...
def divergence(vec, scalar):
    ...
    for i in range(1, num_cells[0]):
        for j in range(1, num_cells[1]):
            for k in range(1, num_cells[2]):
                div_u[i, j, k] \
                    =  (vx[i+1, j, k] - vx[i, j, k])/dx[i] \
                     + (vy[i, j+1, k] - vy[i, j, k])/dy[j] \
                     + (vz[i, j, k+1] - vz[i, j, k])/dz[k]
...
def curl_edge(invec, outvec):
    ...
    #
    # curl(v)_x  @  (x_{i-1/2}, y_{j}, z_{k})
    #
    for i in range(1, num_cells[0]):
        for j in range(1, num_cells[1]):
            for k in range(1, num_cells[2]):
                curlv_x[i, j, k] \
                    =  (vz[i, j+1, k] - vz[i, j, k])/dy[j] \
                     - (vy[i, j, k+1] - vy[i, j, k])/dz[k] 
                     
    #
    # curl(v)_y  @  (x_{i}, y_{j-1/2}, z_{k})
    #
    for i in range(1, num_cells[0]):
        for j in range(1, num_cells[1]):
            for k in range(1, num_cells[2]):
                curlv_y[i, j, k] \
                    =  (vx[i, j, k+1] - vx[i, j, k])/dz[k] \
                     - (vz[i+1, j, k] - vz[i, j, k])/dx[i]
   
    #
    # curl(v)_z  @  (x_{i}, y_{j}, z_{k-1/2})
    #
    for i in range(1, num_cells[0]):
        for j in range(1, num_cells[1]):
            for k in range(1, num_cells[2]):
                curlv_z[i, j, k] \
                    =  (vy[i+1, j, k] - vy[i, j, k])/dx[i] \
                     - (vx[i, j+1, k] - vx[i, j, k])/dy[j]
...
def curl_face(invec, outvec):
    ...
    #
    # curl(v)_x  @  (x_{i}, y_{j-1/2}, z_{k-1/2})
    #
    for i in range(1, num_cells[0]):
        for j in range(1, num_cells[1]):
            for k in range(1, num_cells[2]):
                curlv_x[i, j, k] \
                    =  (vz[i, j, k] - vz[i, j-1, k])/dy[j] \
                     - (vy[i, j, k] - vy[i, j, k-1])/dz[k] 
                     
    #
    # curl(v)_y  @  (x_{i-1/2}, y_{j}, z_{k-1/2})
    #
    for i in range(1, num_cells[0]):
        for j in range(1, num_cells[1]):
            for k in range(1, num_cells[2]):
                curlv_y[i, j, k] \
                    =  (vx[i, j, k] - vx[i, j, k-1])/dz[k] \
                     - (vz[i, j, k] - vz[i-1, j, k])/dx[i]
   
    #
    # curl(v)_z  @  (x_{i-1/2}, y_{j-1/2}, z_{k})
    #
    for i in range(1, num_cells[0]):
        for j in range(1, num_cells[1]):
            for k in range(1, num_cells[2]):
                curlv_z[i, j, k] \
                    =  (vy[i, j, k] - vy[i-1, j, k])/dx[i] \
                     - (vx[i, j, k] - vx[i, j-1, k])/dy[j]
